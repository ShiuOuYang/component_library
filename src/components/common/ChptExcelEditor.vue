<template>
  <div
    ref="containerRef"
    class="chpt-excel-editor w-full rounded-xl border border-gray-200 bg-white shadow-sm overflow-hidden select-none"
  ...
  >
    <!-- ===== 主工具列 ===== -->
    <div v-if="showToolbar" class="border-b border-gray-200 bg-gray-50">
      <div class="flex flex-wrap items-center gap-1 px-2 py-1.5 border-b border-gray-200">
        <span class="px-2 text-xs font-semibold text-gray-600">Excel 編輯器</span>
        ...
        <!-- 數值格式 -->
        <select
          class="h-7 px-2 text-xs text-gray-700 bg-white border border-gray-200 rounded focus:outline-none focus:ring-1 focus:ring-green-500"
          :value="activeNumFmt"
      ...
      <!-- ===== 名稱框 + 公式列 ===== -->
      <div class="relative flex items-center gap-1 px-2 py-1 border-b border-gray-200 bg-white">
        <input
          class="w-24 px-2 py-1 text-xs font-mono text-gray-700 bg-gray-50 border border-gray-300 rounded focus:outline-none focus:ring-1 focus:ring-green-500 text-center"
          :value="activeCellRef"
          readonly
        />
        ...
        <input
          ...
          ref="formulaInputRef"
          class="flex-1 px-2 py-1 text-sm text-gray-800 border border-gray-300 rounded focus:outline-none focus:ring-1 focus:ring-green-500"
        ...
        <div
          ...
          class="absolute z-50 mt-1 w-72 max-h-56 overflow-auto bg-white border border-gray-300 rounded-lg shadow-lg py-1"
          style="top: 100%"
        >
          <div
          ...
          >
            ...
            <span class="text-xs text-gray-500 ml-2">{{ fn.description }}</span>
          </div>
        </div>
    ...
    <div
      v-if="showFxPanel"
      class="border-b border-gray-200 bg-gray-50 px-3 py-2 relative z-40"
    >
      <div class="flex items-center justify-between mb-2">
        <span class="text-xs font-semibold text-gray-600">選擇函式</span>
        <button class="text-xs text-gray-400 hover:text-gray-600" @click="showFxPanel = false">✕</button>
      </div>
      <div class="grid grid-cols-2 md:grid-cols-3 gap-1.5 max-h-40 overflow-auto">
        <button
          v-for="fn in FUNCTION_LIST"
          :key="fn.name"
          class="text-left px-2 py-1.5 text-xs bg-white border border-gray-200 rounded hover:border-green-400 hover:bg-green-50 transition-colors"
          @click="insertFunction(fn.name)"
        >
          <span class="font-mono font-semibold text-green-700">{{ fn.name }}</span>
          <span class="block text-[10px] text-gray-500">{{ fn.description }}</span>
        </button>
      </div>
    </div>
    ...
    <!-- ===== 工作表頁籤 ===== -->
    <div v-if="showSheetTabs" class="flex items-center gap-1 px-2 py-1.5 border-t border-gray-200 bg-gray-50 overflow-x-auto">
...
/* ===== 工具列按鈕 ===== */
.tb-btn {
  @apply inline-flex items-center justify-center w-8 h-7 px-1 text-sm font-medium text-gray-700 bg-white border border-gray-200 rounded transition-colors duration-150;
  @apply hover:bg-gray-100 hover:border-gray-300 disabled:opacity-40 disabled:cursor-not-allowed;
}
.tb-btn.active {
...
.grid-corner {
  @apply sticky z-30 top-0 left-0 w-10 min-w-[40px] h-6 min-h-[24px] bg-gray-100 border border-gray-300;
}
.grid-col-header {
  @apply sticky top-0 text-center text-xs font-semibold text-gray-600 bg-gray-100 border-t border-r border-b border-gray-300 h-6 min-h-[24px] px-1 whitespace-nowrap;
  z-index: 22;
  position: sticky;
  user-select: none;
}
.grid-row-header {
  @apply sticky z-20 left-0 text-center text-xs font-semibold text-gray-600 bg-gray-100 border-l border-r border-b border-gray-300 w-10 min-w-[40px] px-1;
  position: sticky;
  user-select: none;
}
.grid-cell {
  @apply border-r border-b border-gray-200 p-0 m-0 align-middle;
  height: 24px;
  min-height: 24px;
  position: relative;
}
.cell-display {
  @apply block w-full h-full px-1.5 py-0.5 text-sm text-gray-800 overflow-hidden whitespace-nowrap;
  line-height: 1.4;
}
.cell-editor {
  @apply w-full h-full px-1.5 py-0.5 text-sm text-gray-900 border-2 border-green-500 bg-white outline-none;
}
.cell-selected {
...
/* ===== 工作表頁籤 ===== */
.sheet-add {
  @apply flex items-center justify-center w-6 h-6 text-lg font-bold text-gray-500 hover:text-green-600 hover:bg-green-50 rounded transition-colors;
}
.sheet-tab {
  @apply flex items-center gap-1 px-1 py-0.5 text-sm text-gray-600 border border-gray-300 rounded-md cursor-pointer transition-colors;
}
.sheet-tab.active {
...
.sheet-rename-input {
  @apply w-24 px-1.5 py-0.5 text-xs text-gray-700 bg-white border border-green-300 rounded outline-none;
}
</style>