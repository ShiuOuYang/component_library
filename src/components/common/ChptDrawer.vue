          ...
          >
            ...
            >
              ...
              <button
                v-if="props.closable"
                type="button"
                class="flex items-center cursor-pointer text-gray-400 hover:text-gray-600 transition-colors"
                :aria-label="'關閉'"
                @click="handleClose"
              >
            ...
            <div v-if="$slots.footer" class="px-5 py-3 border-t border-gray-100 bg-gray-50">
              <slot name="footer" />
            </div>
          </div>
        </Transition>
...
const panelClasses = computed(() => {
  const map: Record<DrawerPlacement, string> = {
    left: 'left-0 top-0 bottom-0 border-r border-gray-200',
    right: 'right-0 top-0 bottom-0 border-l border-gray-200',
    top: 'top-0 left-0 right-0 border-b border-gray-200',
    bottom: 'bottom-0 left-0 right-0 border-t border-gray-200',
  }
