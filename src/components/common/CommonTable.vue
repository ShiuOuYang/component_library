<template>
  <div class="w-full font-sans mb-5 flex flex-col" :class="[props.containerBgColor, props.containerRounded, props.containerShadow, props.fontSize]">
    <!-- 搜尋和分頁控制區 -->
    <div class="flex justify-between items-center px-3 py-2 border-b border-gray-100" :class="props.controlBgColor">
      <div class="flex items-center gap-2.5">
        ...
        <div class="relative flex items-center">
          <input
            ...
            class="w-48 h-7 px-2.5 border border-gray-300 rounded text-xs text-gray-700 bg-white transition-colors shadow-inner focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100"
    ...
    <!-- 表格內容 -->
    <div class="w-full overflow-x-auto mb-0 border-t border-b border-gray-200">
      <table class="w-full border-collapse text-xs whitespace-nowrap min-w-max">
        <thead>
          <tr>
            <th
              ...
              :style="column.style"
              :class="[
                'bg-gradient-to-b font-medium py-2 px-1.5 text-center border-b border-gray-300 sticky top-0 z-10 whitespace-nowrap tracking-wide shadow-sm',
                props.headerBgGradient,
                props.headerTextColor,
                {
                  'cursor-pointer select-none relative hover:from-blue-50 hover:to-blue-100': isColumnClickable(column),
                  'from-blue-50 to-blue-100': getColumnSortInfo(column.key) !== null,
                  'text-blue-500': getColumnSortInfo(column.key) !== null
                }
              ]"
              ...
              <div class="relative flex items-center justify-center min-h-[28px]">
                ...
                >
                  ...
                  <span
                    ...
                    class="text-[9px] leading-none text-blue-500 font-bold min-w-[10px] text-center"
        ...
        <tbody>
          <tr v-if="paginatedData.length === 0">
            <td :colspan="displayColumns.length" class="py-5 text-center text-gray-500 italic bg-gray-50">{{ noDataText }}</td>
          </tr>
          <template v-else>
            <slot
              name="table-row"
    ...
    <!-- 分頁控制區 (下方) -->
    <div v-if="paginationPosition === 'bottom' || paginationPosition === 'both'" class="flex justify-between items-center px-3 py-2 border-t border-gray-200" :class="props.bottomControlBgColor">
      <div class="flex items-center gap-2.5">
...
const props = withDefaults(defineProps<{
  ...
  headerBgGradient: 'from-gray-50 to-gray-100',
  headerTextColor: 'text-gray-600',
  evenRowBgColor: 'rgb(249 250 251)',
  hoverRowBgColor: 'rgb(239 246 255)',
  controlBgColor: 'bg-white',
  bottomControlBgColor: 'bg-gray-50',
  fontSize: 'text-xs'
})
